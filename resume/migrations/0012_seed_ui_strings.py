# Data migration: UiString jadvaliga standart qiymatlarni yozadi
...
def seed_ui_strings(apps, schema_editor):
    UiString = apps.get_model("resume", "UiString")
    keys = list(TEXTS["en"].keys())
    for key in keys:
        UiString.objects.get_or_create(
            key=key,
            defaults={
                "text_en": TEXTS["en"].get(key, ""),
                "text_uz": TEXTS["uz"].get(key, ""),
                "text_ru": TEXTS["ru"].get(key, ""),
            }
        )


def unseed_ui_strings(apps, schema_editor):
    pass  # rollback da hech narsa o'chirmaymiz


class Migration(migrations.Migration):

    dependencies = [
        ('resume', '0011_add_uistring'),
    ]

    operations = [
        migrations.RunPython(seed_ui_strings, unseed_ui_strings),
    ]
